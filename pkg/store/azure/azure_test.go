...
func newTestBackend(t *testing.T, srvURL string, container string) *Backend {
	t.Helper()
	ctx := context.Background()
	b, err := New(ctx, srvURL, Config{Container: container}, func(o *azblob.ClientOptions) {
...
func TestAzurePutGetStat(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
	ctx := context.Background()

	data := []byte("hello over real Azure Blob wire protocol")
...
func TestAzureRangedGet(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
	ctx := context.Background()
...
func TestAzureRangedGetOpenEnded(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
	ctx := context.Background()
	...
	if string(got) != "abcdefghij" {
		t.Fatalf("open-ended ranged read: got %q, want %q", got, "abcdefghij")
	}
}
...
func TestAzureStatNotFound(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
	ctx := context.Background()
...
func TestAzureConditionalPutProbeDetectsSupport(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
...
func TestAzureConditionalPutRejectsDuplicate(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
	ctx := context.Background()
...
func TestAzureListPrefixAndCursor(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	b := newTestBackend(t, srv.URL, "bkt")
	ctx := context.Background()
	...
	// Paginate with a small max-results and follow the cursor (Azure's own
	// NextMarker, round-tripped opaquely) to prove pagination works
	// end to end over the real wire shape.
...
func TestAzureKeyPrefixIsolation(t *testing.T) {
	// A repo-level "prefix" (e.g. sharing one container across subtrees)
	// must not leak into keys returned to the caller.
	srv := newFakeAzure("bkt")
	defer srv.Close()
	ctx := context.Background()
	b, err := New(ctx, srv.URL, Config{Container: "bkt", Prefix: "tenant-a/"}, func(o *azblob.ClientOptions) {
		o.Retry.MaxRetries = 0
	})
	if err != nil {
		t.Fatal(err)
...
// TestAzureCapsCostFields proves EgressUSDPerGB/GetUSDPer1k/PutUSDPer1k are
// caller-supplied, feeding the cost model (DESIGN.md §12) rather than a
// hardcoded pricing table baked into this package.
func TestAzureCapsCostFields(t *testing.T) {
	srv := newFakeAzure("bkt")
	defer srv.Close()
	ctx := context.Background()
	b, err := New(ctx, srv.URL, Config{
		Container:      "bkt",
		EgressUSDPerGB: 0.087,
		GetUSDPer1k:    0.0004,
		PutUSDPer1k:    0.005,
	}, func(o *azblob.ClientOptions) {
		o.Retry.MaxRetries = 0
	})
	if err != nil {
		t.Fatal(err)
	...
	caps := b.Caps()
	if caps.EgressUSDPerGB != 0.087 || caps.GetUSDPer1k != 0.0004 || caps.PutUSDPer1k != 0.005 {
		t.Fatalf("cost fields not carried through Caps(): %+v", caps)
	}
}
