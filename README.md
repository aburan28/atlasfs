...
- **Permissions that survive a round trip** — `atlas publish` stored a hardcoded `0644` for every file, so publishing a tree of scripts or binaries and mounting it gave back files you could not run. The source's own bits are now preserved, `chmod` through either mount persists (and reaches other holders through the same invalidation path a write takes), and an overwrite no longer strips the bits back off — a commit carries content, not permissions, and taking its mode verbatim is how "chmod +x then edit" silently breaks. A read-only mount clears the write bits but deliberately keeps the exec bit, since running a read-only published tree is the point. Ownership is still accepted-and-ignored: §20's uid/gid model is a later phase, and storing an owner ahead of it would create a second source of truth to reconcile.
