package gcs

// Proves the Backend isn't just correct in isolation (gcs_test.go) but
// actually carries a full publish + read cycle through pkg/repo over the
// real cloud.google.com/go/storage wire protocol — DESIGN.md §24's
// pluggability claim, verified rather than asserted.

import (
	"bytes"
	"context"
	"net/http"
	"os"
	"path/filepath"
	"testing"

	"google.golang.org/api/option"

	"github.com/aburan28/atlasfs/pkg/repo"
)

func TestPublishAndReadThroughGCSBackend(t *testing.T) {
	srv := newFakeGCS("atlas-bucket")
	defer srv.Close()

	ctx := context.Background()
	backend, err := New(ctx, Config{Bucket: "atlas-bucket"},
		option.WithEndpoint(srv.URL),
		option.WithoutAuthentication(),
		option.WithHTTPClient(&http.Client{}),
	)
	if err != nil {
		t.Fatal(err)
	}

	r, err := repo.OpenRemote(t.TempDir(), backend, "test-region")
	if err != nil {
		t.Fatal(err)
	}
	defer r.Close()
	r.ChunkSize = 4096

	src := t.TempDir()
	data := bytes.Repeat([]byte("gcs-backed-content-"), 2000) // multi-chunk
	if err := os.MkdirAll(filepath.Join(src, "dir"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(src, "dir", "f.bin"), data, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(src, "small.txt"), []byte("small"), 0o644); err != nil {
		t.Fatal(err)
	}

	files, _, _, err := r.PublishTree(ctx, src, []string{"datasets", "demo"})
	if err != nil {
		t.Fatal(err)
	}
	if files != 2 {
		t.Fatalf("expected 2 published files, got %d", files)
	}

	_, rec, err := r.Resolve("/datasets/demo/dir/f.bin")
	if err != nil {
		t.Fatal(err)
	}
	fr, err := r.OpenFile(ctx, rec)
	if err != nil {
		t.Fatal(err)
	}
	got, err := fr.ReadAll()
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(got, data) {
		t.Fatal("content mismatch reading a multi-chunk file back over the GCS backend")
	}

	_, smallRec, err := r.Resolve("/datasets/demo/small.txt")
	if err != nil {
		t.Fatal(err)
	}
	if !smallRec.HasInline {
		t.Fatal("small file should still be inlined even with a remote backend")
	}
	fr2, err := r.OpenFile(ctx, smallRec)
	if err != nil {
		t.Fatal(err)
	}
	gotSmall, err := fr2.ReadAll()
	if err != nil {
		t.Fatal(err)
	}
	if string(gotSmall) != "small" {
		t.Fatalf("got %q, want %q", gotSmall, "small")
	}
}
