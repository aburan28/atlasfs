...
Not yet built, and why each is a real gap rather than an oversight: FoundationDB, multi-region homing, the namespace map, and rehoming (§7 — needs a running FDB cluster; `pkg/mds` is the client/authority boundary those would sit on, not the topology itself); byte-range locks and cross-node `flock`/`fcntl` forwarding (§17 — the RWX `disjoint-writers` carve-out only provides local mutual exclusion, see `TestRWXDisjointWritersFlockIsLocalOnly`); in-place random-access writes (§16.3 — both mounts buffer a file and commit it as a unit on flush); a CSI Controller service and dynamic provisioning (§22 — needs a real cluster); client-side write-session `ESTALE` enforcement, the other half of GC-1 (§19.2), and *leased* open handles for the authority-backed mount (§19.3 — the in-process registry is done and sufficient there, see `pkg/repo/gc.go`'s package doc); `FIEMAP` and `O_DIRECT` (`fallocate` is implemented, but `generic/009` still needs the extent-mapping ioctl); the §21.2 client performance work the benchmarks above quantify; §27's 24-hour `fsx` soak and the applicable `generic/` groups of its xfstests suite (docs/conformance.md records what pjdfstest, `fsx` and the quick group did cover); and the NFS export (§23 — needs `nfs-ganesha`, not installed here).
