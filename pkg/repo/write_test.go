...
func TestImmutableRepoRejectsWrites(t *testing.T) {
	...
	if _, err := r.Mkdir(metadb.RootInode, "d", 0o755, Owner{}); !errors.Is(err, ErrReadOnly) {
		t.Fatalf("expected ErrReadOnly, got %v", err)
	}
...
func TestMkdirRmdir(t *testing.T) {
	...
	id, err := r.Mkdir(metadb.RootInode, "sub", 0o755, Owner{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRmdirRefusesNonEmpty(t *testing.T) {
	...
	if _, err := r.Mkdir(metadb.RootInode, "sub", 0o755, Owner{}); err != nil {
		t.Fatal(err)
	}
...
func TestCommitRefusesToOverwriteADirectory(t *testing.T) {
	...
	if _, err := r.Mkdir(metadb.RootInode, "d", 0o755, Owner{}); err != nil {
		t.Fatal(err)
	}
...
func TestMkdirRefusesExistingName(t *testing.T) {
	...
	if _, err := r.Mkdir(metadb.RootInode, "f.txt", 0o755, Owner{}); err == nil {
...
func TestQuotaRejectsMkdirOverInodeLimit(t *testing.T) {
	...
	if _, err := r.Mkdir(metadb.RootInode, "first", 0o755, Owner{}); err != nil {
		t.Fatalf("first mkdir within the inode limit should succeed, got %v", err)
	}
	if _, err := r.Mkdir(metadb.RootInode, "second", 0o755, Owner{}); !errors.Is(err, ErrQuotaExceeded) {
		t.Fatalf("expected ErrQuotaExceeded, got %v", err)
	}
